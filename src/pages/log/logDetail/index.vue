  ...
  <div class="elocLog-detail">
    ...
    <div class="elocLog-detail-content">
      ...
      <div class="elocLog-detail-content-chart" v-show="!showTable">
        <v-chart
          id="elocLogDetailChart"
          :manual-update="true"
          ref="elocLogDetailChart"
        />
      </div>
    </div>
  </div>
...
<script>
  ...
  import {getLogTableDetailData, exportExcel, getLogChartDetail, getLogChartDetailData} from 'api/log'
  ...
  export default {
    ...
    methods: {
      ...
      loadTable() {
        ...
        if (this.tableData.length === 0) {
          ...
          getLogTableDetailData(params).then(res => {
            this.tableData = res.msg.dataList;
            this.loading = false
          })
        }
      },
      loadChart() {
        ...
        getLogChartDetailData(params).then(res => {
          if (res) {
          ...
          getLogChartDetail(params).then(res => {
            ...
            for (var i = 0; i < res.msg.length; i++) {
              for (var ii = 0; ii < res.msg[i].length; ii++) {
                if (res.msg[i][ii][0] && res.msg[i][ii][1] && res.msg[i][ii][2]) {
                  let opt = {
                    name: xxbh[i],
                    data: res.msg[i],
                    type: 'custom'
