...
export default {
  ...
  methods: {
    ...
    initMapConfiguration() {
      this.$refs.elocMap.initElocMap().then(() => {
        this.getCrossList().then(() => {
          addSocketSubscribe(
            "/exchange/UTC/UTC_SLC_CONTROL",
    ...
    setMaker(crossList) {
      crossList.forEach((i, n) => {
        ...
        window.gis.OverlayUtil.bindEvt(marker, "mouseover", e =>
          this.showCrossInfoPopu(e, i.crossNo)
        );
      });
    },
    //      写入鼠标移入事件
      ...
      getCrossTimingPlan({ crossNo: crossNo, planNo: "0" }).then(res => {
        ...
        timingPlanInfo.forEach((i, n) => {
          timePlan +=
            xxbh[n] +
            ":" +
            (parseInt(i.green) + parseInt(i.yellow) + parseInt(i.allRed)) +
            "s, ";
          cycle +=
            parseInt(i.green) + parseInt(i.yellow) + parseInt(i.allRed) + "s";
        });
    ...
    areaSelect() {
      ...
      this.drawPolygonOnMap(
        ...
        polygon => {
          ...
          const wkt = this.$refs.elocMap.latLngs2Wkt(
            "polygon",
            polygonPoints[0]
          );
    ...
    getDrawingTool() {
      if (this.areaSelectContainer.drawTool === null) {
        this.areaSelectContainer.drawTool = new window.gis.DrawingUtil(
          this.mapContext,
          {
            polygonOptions: {
              stroke: true, //边线颜色。
    ...
    getCrossInfoBySpatialSearch() {
      ...
      getCrossInfoBySpatialSearch({ wkt })
        .then(res => {
          if (res.code === 0) {
            ...
            this.statistics = {
              ...spatialCountData,
              errorCount:
                parseInt(spatialCountData.cannotPingCount) +
                parseInt(spatialCountData.cannotConnectCount)
            };
    ...
    batchMoveRight() {
      this.batchSelectList = this.$refs.batchTree
        .getCheckedNodes()
        .filter(i => !i.children);
    },
    batchMoveLeft() {
    ...
    confirmBatch() {
      ...
      const content = `执行${this.batchSelectList.map(
        i => i.title
      )}的 ${transferOperationDesc(this.batchRadio)} 操作`;
      this.$confirm({
        title: "是否执行以下操作？",
        content,
